...
import { Search, Eye, CheckCircle, Clock, AlertTriangle, Loader2, AlertCircle, X, Car, Trash2 } from 'lucide-react';
...
export default function RentalsPage() {
  ...
  const handleAddDamageInvoice = async (e) => {
    e.preventDefault();
    if (!damageDesc || !damageCost || isNaN(damageCost) || Number(damageCost) <= 0) {
    ...
    try {
      ...
      setSelected(data);
      alert('Damage repair invoice created successfully, and the customer has been notified!');
    } catch (err) {
  ...
  const handleDeleteDamageInvoice = async (invoiceId) => {
    if (!window.confirm('Are you sure you want to delete this damage invoice?')) return;
    try {
                    ...
                    <tr key={b.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-0.5">
                          <button onClick={() => handleOpenDrawer(b)} className="p-1.5 text-gray-400 hover:text-[#2D6A4F] rounded-md transition-colors">
            ...
            {loadingDetails ? (
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {selected.rental && (
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="text-[10px] font-semibold text-gray-400 uppercase mb-2">Invoices & Charges</div>
                      <div className="space-y-2">
                        ...
                        {selected.rental.invoice && (
                          <div className="flex items-center justify-between text-xs border-b border-gray-100 pb-2">
                            <div>
                              <span className="font-semibold text-gray-700">Rental Charge</span>
                              <div className="text-[9px] text-gray-400">Invoice #{selected.rental.invoice.id}</div>
                            </div>
                            <div className="text-right">
                              <span className="font-bold text-gray-800">${parseFloat(selected.rental.invoice.total).toFixed(2)}</span>
                        ...
                        {(selected.rental.invoices || [])
                          ...
                          .map(inv => (
                            <div key={inv.id} className="flex items-start justify-between text-xs border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                              <div>
                                <span className="font-semibold text-red-600">🚨 Repair Charge</span>
                                <p className="text-[10px] text-gray-600 leading-tight mt-0.5">{inv.description}</p>
                                <div className="text-[9px] text-gray-400">Invoice #{inv.id}</div>
                              </div>
                              <div className="text-right shrink-0 flex items-center gap-2">
                                <div>
                                  <span className="font-bold text-gray-800">${parseFloat(inv.total).toFixed(2)}</span>
                                  <div className="text-[9px] text-amber-600 font-semibold uppercase">Unpaid</div>
                                </div>
                                <button
                                  onClick={() => handleDeleteDamageInvoice(inv.id)}
                  ...
                  {selected.rental && (
                    <div className="bg-red-50/50 border border-red-100 rounded-lg p-4">
                      <div className="text-[10px] font-bold text-red-800 uppercase tracking-wider mb-2">Record Vehicle Damage</div>
                      <form onSubmit={handleAddDamageInvoice} className="space-y-3">
                        <div>
                          <label className="block text-[9px] font-bold text-gray-500 uppercase mb-1">Damage Description</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[9px] font-bold text-gray-500 uppercase mb-1">Repair Cost ($)</label>
                          <input
                            type="number"
                        ...
                        >
                          {submittingDamage ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : 'Issue Repair Invoice & Notify'}
                        </button>
                      </form>
                    </div>
                  )}
