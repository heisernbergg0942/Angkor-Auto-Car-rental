...
export default function LoginPage() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // TODO: Implement actual login logic with context
    // For now, just route to customer portal
    navigate('/');
  };
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 relative">
        ...
        <div className="absolute top-8 left-8 lg:hidden">
          <Link to="/" className="text-xl font-bold text-gray-900 flex items-center gap-2">
            <div className="w-6 h-6 bg-[#2D6A4F] rounded-md flex items-center justify-center">
              <span className="text-white text-sm leading-none">F</span>
            </div>
            FleetRent
          </Link>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <a href="#" className="text-xs font-semibold text-[#2D6A4F] hover:text-[#1B4332]">Forgot password?</a>
              </div>
              <div className="relative">
            ...
            <button 
              type="submit" 
              className="w-full bg-[#2D6A4F] text-white font-semibold py-2.5 rounded-lg hover:bg-[#1B4332] transition-colors flex items-center justify-center gap-2 mt-4"
            >
              Sign In
          ...
          <div className="mt-8 text-center text-sm text-gray-500 border-t border-gray-100 pt-8">
            ...
            <Link to="/register" className="font-semibold text-[#2D6A4F] hover:text-[#1B4332]">
              Create one now
            </Link>
          </div>
        </div>
