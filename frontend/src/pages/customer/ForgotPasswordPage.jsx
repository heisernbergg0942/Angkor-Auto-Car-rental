...
export default function ForgotPasswordPage() {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      setMessage(data.message || 'Password reset link sent! Check your email or local logs.');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to send reset link. Ensure the email exists.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex text-gray-900 bg-white">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center relative p-8 sm:p-12 lg:px-24">
        ...
        <div className="w-full max-w-md mx-auto flex-1 flex flex-col justify-center">
          ...
          {message && (
            <div className="bg-emerald-50 border border-emerald-200 text-emerald-600 text-xs px-4 py-3 rounded-lg mb-4">
              {message}
            </div>
          )}
          {error && (
